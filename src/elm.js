...
Elm.Character.make = function (_elm) {
   ...
   var moodCount = function (c) {
      var _p0 = c;
      switch (_p0.ctor)
      {case "Toriel": return 0;
         case "Sans": return 0;
         case "Papyrus": return 0;
         case "Undyne": return 0;
...
Elm.UndertaleDialog.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var noop = function (whatever) {    return whatever;};
   var update = F2(function (action,model) {
      var _p0 = action;
      switch (_p0.ctor)
      {case "ChooseCharacter": return _U.update(model,{selection: $Maybe.Just(_p0._0)});
         case "ChooseMood": return noop(model);
         default: return noop(model);}
   });
   var EnterText = {ctor: "EnterText"};
   var ChooseMood = {ctor: "ChooseMood"};
   var ChooseCharacter = function (a) {    return {ctor: "ChooseCharacter",_0: a};};
   var moodHeader = A2($Html.h1,_U.list([]),_U.list([$Html.text("Mood")]));
   var spriteFolder = function (c) {    return A2($Basics._op["++"],"sprites",A2($Basics._op["++"],"/",$Basics.toString(c)));};
   var spriteNumber = F2(function (c,n) {
      return A2($Basics._op["++"],spriteFolder(c),A2($Basics._op["++"],"/",A2($Basics._op["++"],$Basics.toString(n),".png")));
   });
   var defaultSprite = function (c) {    return A2(spriteNumber,c,0);};
   var characterButton = F2(function (address,c) {
      return A2($Html.button,
      _U.list([A2($Html$Events.onClick,address,ChooseCharacter(c))]),
      _U.list([A2($Html.img,_U.list([$Html$Attributes.src(defaultSprite(c))]),_U.list([]))]));
   });
   var characterButtons = F2(function (address,characters) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.ul,_U.list([$Html$Attributes.$class("characters")]),A2($List.map,characterButton(address),characters))]));
   });
   var moodButton = F2(function (c,n) {
      return A2($Html.button,_U.list([]),_U.list([A2($Html.img,_U.list([$Html$Attributes.src(A2(spriteNumber,c,n))]),_U.list([]))]));
   });
   var moodButtons = function (c) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.ul,_U.list([$Html$Attributes.$class("moods")]),A2($List.map,moodButton(c),_U.range(0,$Character.moodCount(c) - 1)))]));
   };
   var moodSection = function (maybeChar) {
      var _p1 = maybeChar;
      if (_p1.ctor === "Nothing") {
            return A2($Html.div,_U.list([]),_U.list([]));
         } else {
            return moodButtons(_p1._0);
         }
   };
   var characterHeader = A2($Html.h1,_U.list([]),_U.list([$Html.text("Character")]));
   var view = F2(function (address,model) {
      return A2($Html.div,_U.list([]),_U.list([characterHeader,A2(characterButtons,address,model.characters),moodHeader,moodSection(model.selection)]));
   });
   var init = F2(function (characters,selection) {    return {characters: characters,selection: selection};});
   var Model = F2(function (a,b) {    return {characters: a,selection: b};});
...
Elm.Main.make = function (_elm) {
   ...
   $UndertaleDialog = Elm.UndertaleDialog.make(_elm);
   var _op = {};
   var main = $StartApp$Simple.start({model: A2($UndertaleDialog.init,_U.list([$Character.Alphys]),$Maybe.Nothing)
                                     ,update: $UndertaleDialog.update
                                     ,view: $UndertaleDialog.view});
